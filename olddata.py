...
for dir in dataset_dirs:
    ...
    try: 
        if dir.split("_")[2] == "COSMOS":
            model = "COSMOS-ASO"
        else:
            model = "MPI-ESM"
    except:
        if dir.split("_")[0] == "Pliocene" or dir.split("_")[0] == "Miocene":
            model = model = "COSMOS-ASO"
        else:
            model = "COSMOS-AO"

    marker = ""
    epoch = dir.split("_")[0]
    if epoch == "PreIndustrial":
        marker = dir.split("_")[1] + dir.split("_")[2]
    epochs.append(epoch + marker)
    epoch_ref ="PreIndustrial"
    x = dir.split("_")[1].replace("xCO2", "")

    # set correct var names
    exp_file_name_template = rootdir + dir + "/data/MODEL_EPOCH_XxCO2_100years.nc" 
    ref_file_name_template = rootdir + dir + "/data/MODEL_PreIndustrial_100years.nc"
    ...
    exp_precip_yearmeans_fldmean=exp_file_hdl.variables['precip'][:].squeeze() # global avg time series for further tasks
    ref_precip_yearmeans_fldmean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    exp_precip_timmean_fldmean=exp_file_hdl.variables['precip'][:].squeeze()
    ref_precip_timmean_fldmean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    exp_precip_yearmeans_lowlat_mean=exp_file_hdl.variables['precip'][:].squeeze()
    ref_precip_yearmeans_lowlat_mean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    exp_precip_timmean_lowlat_mean=exp_file_hdl.variables['precip'][:].squeeze()
    ref_precip_timmean_lowlat_mean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    exp_precip_yearmeans_arctic_mean=exp_file_hdl.variables['precip'][:].squeeze()
    ref_precip_yearmeans_arctic_mean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    exp_precip_timmean_arctic_mean=exp_file_hdl.variables['precip'][:].squeeze()
    ref_precip_timmean_arctic_mean=ref_file_hdl.variables['precip'][:].squeeze()
    ...
    cs = (exp_temp2_timmean_fldmean-ref_temp2_timmean_fldmean)/(2/x)
...
fig, ax = plt.subplots()
for pair in zip(climate_sens, arctic_amp, epochs, climate_sens_err, arctic_amp_err):
...
fig.savefig("/home/max/code/KollegAG3ModellAuswertung/out_olddata/phasespace.pdf")

fig, ax = plt.subplots()
for pair in zip(arctic_amp, antarctic_amp, epochs, arctic_amp_err, antarctic_amp_err):
    ax.errorbar(pair[0], pair[1], xerr=pair[3], yerr=pair[4], label=pair[2], fmt="v", markersize=8, capsize=5, ecolor="black")
...
fig.tight_layout()
fig.savefig("/home/max/code/KollegAG3ModellAuswertung/out_olddata/phasespace2.pdf")

import csv
with open('/home/max/code/KollegAG3ModellAuswertung/out_olddata/res.txt', 'w') as f:
      
    # using csv.writer method from CSV package
    write = csv.writer(f)
      
    write.writerow(["Epoch/Model", "CS", "δCS", "AA", "δAA", "AAA", "δAAA"])
    write.writerow([epochs, climate_sens, climate_sens_err, arctic_amp, arctic_amp_err, antarctic_amp, antarctic_amp_err])
